    ...
    >
        ...
        <div
            ...
            <div v-if="blocks" class="app-sidebar-blocks">
                <ul>
                    <!-- 单个区块 -->
                    <li v-for="(block, index) in list" :key="index" class="app-sidebar-block"  @click.stop="closeAndGo(block.id)">
                        <div v-if="block.title" class="sub-list-title">{{ block.title }}<div class="myimg"><img v-if="block.icon" :src="block.icon"/></div></div>
                        <!-- <ul v-if="block.list">
...
import { health_list } from '@/model/api.js'
